...
def split_stock_data(json_data: json) -> dict:
    ...
    for company in json_data["aaData"]:
        c_name, stock_code, time, price, var_day, _, _, _, min_value, max_value, _, volume = company
        stock_list.append((None, stock_code, c_name, f"{current_time} {time}", price, min_value, max_value, volume))
    return stock_list
